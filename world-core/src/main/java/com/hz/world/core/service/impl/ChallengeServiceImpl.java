...
public class ChallengeServiceImpl implements ChallengeService {
	...
	public ResultDTO<String> challenge(Long userId, Integer element,Integer elementWeight, Integer totalWeight) {
		...
		try {
			/*
			1. 根据元素和体重，config里面查找到的目前能达到的成就，
			2. log里面判断该成就是否已经领取，
			3. 如果达到新的成就没有领取，则获取成就，增加元素的总体收益，UserElementService里的addElementAdd方法，然后记录日志
			4. 计算总的体重的成就和奖励UserElementService的addTotalAdd方法
			*/
			...
			if(configList != null && configList.size() > 0 )
			{
				for(ChallengeConfig config:configList) {
					if(config.getElement().equals(element) && config.getWeight()<=elementWeight) {
						...
						// 2. log里面判断该成就是否已经领取
						if(userChallengeLogList!=null && userChallengeLogList.size()>0){
