...
public class ChallengeServiceImpl implements ChallengeService {
	...
	public ResultDTO<String> challenge(Long userId, Integer element, Integer elementWeight, Integer totalWeight) {
		...
		try {
			...
			List<UserChallengeLog> userChallengeLogList = userChallengeLogDao.getUserFinishedChallenges(userId,
					element);
			// 1-2 查找目前能到达的成就
					...
					if (config.getElement().equals(element) && config.getWeight() <= elementWeight) {
						...
						if (!isInList(config.getId(), userChallengeLogList)) {
							...
							if (config.getRewardElement().equals(11)) {
								userElementService.addTotalAdd(userId, ElementAdd.CHALLENGE.getCode(),
										config.getPower());
							}
							// 单元素收益
							else {
								userElementService.addElementAdd(userId, config.getRewardElement(),
										ElementAdd.CHALLENGE.getCode(), config.getPower() + "");
							}
							// 增加日志记录
